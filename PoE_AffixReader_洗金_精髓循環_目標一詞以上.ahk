...
;------設定目標詞綴------
global affix := Array()
affix.Push(["\+(3[6789]|4[01])% to Cold Resistance","(3[6789]|4[01])% to fire Resistance"])
	...
	loop
		...
		IfWinActive,Path of Exile 
			...
			;若滑鼠為空,物品稀有度為普通,點精髓
				...
				;確保滑鼠上沒捏東西
				Send {LShift Up}
				MouseClick, Right,Essence_X,Essence_Y,1,delay
				MouseClick, Left,ItemPos[0],ItemPos[1],1,delay
			...
			;若滑鼠為空,物品為稀有,點精髓並把精髓捏在手上
			else if (LShiftState = 0 && mouseState = 0 && itemRarity = 2 && Increased = 0)
				{	
				Send {LShift Up}			
				Send {LShift Down}
				LShiftState := 1
