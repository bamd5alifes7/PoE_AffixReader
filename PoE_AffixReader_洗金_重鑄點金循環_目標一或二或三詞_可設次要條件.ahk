...
;------設定目標詞綴------
global affix := Array()
;火傷星團找燃燒閃耀 洗星團會跳過其餘三天，有點奢侈
;affix.Push(["Burning Bright"])

affix.Push(["to maximum Life"])

/*
找到主要目標之後，篩選是否有次要目標。須配合設定
targetAffixNum和targetSecondAffixNum的值來決定主要目標需要找到幾個與次要目標想要找到幾個。
*/

;------設定次要目標詞綴------
global secondAffix := Array()

;secondAffix.Push(["Sadist","Corrosive Elements","Doryani's Lesson","Disorienting Display","Prismatic Heart","Widespread Destruction","Master of Fire","Smoking Remains","Cremator"])
secondAffix.Push(["Fire Resistance","Cold Resistance","Lightning Resistance","Chaos Resistance","Damage taken Recouped as Life","all Elemental Resistances","Master of Fire","to Dexterity","to Intelligence"])
...
scouringAlchemyLoop()
	...
	;檢查是否有次要目標，並記錄檢查值
	if (secondAffix.MaxIndex() > 0 && secondAffix[1].Length() > 0) 
		{
		secondAffixCheck = 1
		}
	...
	loop
		...
		IfWinActive,Path of Exile 
			...
			;確認此前的程式狀態。Shift沒按下、滑鼠沒捏東西、沒用過增幅石,操作重鑄點金
			...
			;假如有設置secondAffix的內容，才取得判斷值
			if (secondAffixCheck = 1) 
				{
				conformSecondNum = % getConformSecondAffix()
				}
			...
			if % ConformNum = 1
				...
				if % itemRarity = 2
					...
					if % targetAffixNum = 1
						{
						;沒設置次要目標的話，break
						if % secondAffixCheck = 0
							{
							break
							}
						
						;有設置次要目標的話，再檢查conformSecondNum，依照需求停止
						else if % secondAffixCheck = 1
							{
							if conformSecondNum = 1
								{
								if targetSecondAffixNum = 1
									{
									break
									}						
								}
								
							if conformSecondNum = 2
								{
								if targetSecondAffixNum = 2
									{
									break
									}						
								}
								
							if conformSecondNum = 3
								{
								if targetSecondAffixNum = 3
									{
									break
									}						
								}								
