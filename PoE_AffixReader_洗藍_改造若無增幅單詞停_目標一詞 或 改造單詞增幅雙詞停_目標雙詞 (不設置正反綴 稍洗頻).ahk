...
~F4:: alterationAugmentationLoop()
F7:: saveCoordinatesTool()



alterationAugmentationLoop()
	{
	...
	loop
		...
		IfWinActive,Path of Exile 
			...
			;根據符合詞綴數、目標詞綴數、裝備稀有度進行動作
			;因點完改造後有可能需要點增幅。此處為第一次判斷。
				...
				if % itemRarity = 1
					...
					if %  targetAffixNum = 1
						...
						if Increased = 0
							...
							;複製物品資訊，檢查是否和操作前一樣，不一樣才離開。最大3圈，因為很有可能操作完會一模一樣。
							loop, 3
								{
								send ^c
								send ^c						
					...
					;如果targetAffixNum大於2，卻連單詞都沒有，不點增幅石
					else if % targetAffixNum >= 2
							{		
							continue
							}
					}
			...
			;如果ConformNum = 2，targetAffixNum又在2以下，代表點完增幅後完成目標
			else if % ConformNum = 2
				{
				if % itemRarity = 1
					{
